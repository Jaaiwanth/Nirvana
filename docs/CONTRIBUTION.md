...
| **Person 3** | **AI & Agent Engineer** | `backend/src/agents/`<br/>`backend/src/ai/`<br/>`tests/scenarios/` | • Groq SDK integration (`llama-3.3-70b-versatile`) & Gemini 2.0 Flash adapter.<br/>• Incident Extraction Agent (Zod schema for type, severity, capabilities).<br/>• Multi-Criteria Decision Agent (scoring ETA vs. equipment match).<br/>• Deterministic rule-based fallback heuristic engine.<br/>• 10 realistic disaster scenario benchmark test suite. |
